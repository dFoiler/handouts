...
\end{theorem}
\begin{proof}
	The argument is similar to the proof of \Cref{thm:weil-converse}. For example, to show
...
\end{lemma}
\begin{proof}
	\Cref{lem:diff-operator-ids} shows $R_k$, $L_k$, and $\Delta_k$ send $C^\infty(\Gamma\backslash\HH,\chi,k)$ to $C^\infty(\Gamma\backslash\HH,\chi,k+2)$, $C^\infty(\Gamma\backslash\HH,\chi,k-2)$, and $C^\infty(\Gamma\backslash\HH,\chi,k)$, respectively. In fact, letting $w\coloneqq\gamma z$ as usual for some $\gamma\in G$ where $z=x+iy$ and $w=u+iv$, an application of Stokes's theorem to the differential form
...
To answer \Cref{ques:spectral-laplacian}, we introduce the following definition.
\begin{definition}[Maass form]
...
\begin{proof}
	...
	\[f(x+yi)\coloneqq F\left(\begin{bmatrix}
		y & x \\ & 1
	\end{bmatrix}\right).\]
	Lastly, the compatibility of inner products can be checked from \Cref{ex:iwasawa-haar-measure}.
\end{proof}
We are now able to move $R$, $L$, and $\Delta$ from $L^2(\Gamma\backslash\HH,k)$ to $L^2(\Gamma\backslash G,\chi)$. Throughout, we use the Isawasa coordinates $(u,x,y,\theta)$ provided in \Cref{ex:iwasawa-haar-measure}. Then we define
\begin{align*}
