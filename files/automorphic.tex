...
\Cref{thm:classical-chaos} is ``classical chaos.'' We would like a formulation in quantum mechanics. To do this mathematically, we begin with a $C^*$-algebra $\mc A$, which is a subalgebra of some bounded (i.e., continuous) operators on a Hilbert space, complete with respect to the norm and closed under the adjoint.
\begin{example}
...
\end{theorem}
This was originally proven under the assumption that we can remove a natural density $0$ subset of ``exceptional'' eigenfunctions. To explain why this is hard, there is a geodesic on $\op{SL}_2(\ZZ)$ which more or less fails to equidistribute. %\todo{Why is exceptoinal helpful?}
...
\end{itemize}
% \begin{remark}
% 	Let's give a physical interpretation of what's going on in the non-compact case. The cuspidal subspace of $L^2(\Gamma\backslash\HH)$ has a discrete spectrum. 
% \end{remark}
...
\end{remark}
The point is that one is able to use these sorts of decompositions (and in particular our Parseval's identity) to prove \Cref{thm:quantum-chaos} in our case.
...
\begin{definition}[Levi subgroup]
...
\begin{definition}[Whittaker function, Whittaker model]
...
\begin{definition}[global Whittaker function]
...
\begin{definition}[local Rankin--Selberg integral]
...
\begin{definition}[global Rankin--Selberg $L$-function]
