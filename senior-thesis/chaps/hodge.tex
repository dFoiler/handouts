...
\begin{notation}
	...
	for any ring $R$. Frequently, we will have fixed once and for all an embedding of $K$ into $\CC$, so we may abbreviate $\mathrm H^i_\sigma(X,R)$ to just $\mathrm H^i_{\mathrm B}(X,R)$.
\end{notation}
...
\begin{theorem} \label{thm:betti-etale-comparison}
	...
	\[\mathrm H^i_{\mathrm B}(X,\QQ_\ell)\to\mathrm H^i_{\mathrm{\acute et}}(X,\QQ_\ell).\]
\end{theorem}
...
\end{remark}
The above cohomology theories have mostly worked in characteristic $0$, but it is notable that \'etale cohomology has managed to work with $\op{char}K\nmid\ell$. This does leave a gap in positive characteristic, where we may want to define a cohomology ``at'' $\op{char}K=\ell$. This is achieved by crystalline cohomology; we will not use crystalline cohomology very much, but we go ahead and give some of its basic properties. We refer to \cite{illusie-crystalline} for a more thorough review.
\begin{defihelper} \nirindex{cohomology!crsytalline cohomology}
	Fix a smooth projective variety $X$ over a perfect field $K$ of positive characteristic $p$. Let $W$ be the ring of Witt vectors over $K$, and define $W_n\coloneqq W/p^nW$ for each $n\ge0$. Then we define the \textit{crystalline cohomology} $\mathrm H^\bullet_{\mathrm{crys}}(X,W)$ as
	\[\mathrm H^i_{\mathrm{crys}}(X,W)\coloneqq\limit\mathrm H^i_{\mathrm{crys}}(X,W_n),\]
	where the right-hand cohomology is the cohomology on the crystalline site of $X$ over $W_n$.
\end{defihelper}
\begin{remark}
	Because $X$ has an absolute Frobenius, the crystalline cohomology groups come with the action by an absolute Frobenius.
\end{remark}
\begin{theorem}
	Fix a smooth projective variety $X$ over a perfect field $K$ of positive characteristic $p$, and set $W\coloneqq W(K)$. If there is a smooth projective variety $Z$ over $W$ such that $Z_K=X$, then there is a natural isomorphism
	\[\mathrm H^\bullet_{\mathrm{crys}}(X,W)\to\mathrm H^\bullet_{\mathrm{dR}}(Z,W).\]
\end{theorem}
